...
The `wget` command is a tool to download files from the web. It's a very powerful tool, but we will
...
The `curl` command is a tool to download files from the web. While *wget* will save the file with its
...
## Before we go on: installing HomeBrew
